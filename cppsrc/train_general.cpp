...
//                std::cout << std::endl;
    ...
    for (size_t i = 0; i < m_features; ++i) {
        if (l_dimensions > 0) {
            ...
            output_file << b_weights[index_b_weights(i, l_dimensions - 1)] <<
            std::endl;
        }
    }
    ...
    for (size_t i = 0; i < m_features; ++i) {
        if (k_dimensions > 0) {
            ...
            output_file << c_weights[index_c_weights(i, k_dimensions - 1)] <<
            std::endl;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    for (int d = 0; d <= l_dimensions; ++d) {
        for (int k = 0; k <= k_dimensions; ++k) {
            for (int i = 1; i <= fold_number; ++i) {
                train_with_features(
                        (boost::format(
                                "/home/diegob/workspace/master-thesis-2015/data/path_set_nce_data_features_%1%_fold_%2%.csv") %
                         feature_set % i).str(),
                        (boost::format(
                                "/home/diegob/workspace/master-thesis-2015/data/path_set_nce_features_%1%.csv") %
                         feature_set).str(),
                        (boost::format(
                                "/home/diegob/workspace/master-thesis-2015/data/path_set_nce_noise_features_%1%_fold_%2%.csv") %
                         feature_set % i).str(),
                        10, 0.01, 0.1, static_cast<size_t>(d), static_cast<size_t>(k),
                        (boost::format(
                                "/home/diegob/workspace/master-thesis-2015/data/models/path_set_nce_out_features_%1%_l_dim_%2%_k_dim_%3%_fold_%4%.csv") %
