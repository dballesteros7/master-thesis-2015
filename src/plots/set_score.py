...
def plot_scores():
    ...
    pseudo_markov, pseudo_markov_std, _, __ = rank_results('pseudo_markov',
                                                           N_ITEMS)
    modular, modular_std, _, __ = rank_results('modular_features_0', N_ITEMS)
    submod, submod_std, _, __ = rank_results('submod_f_0_l_20_k_20', N_ITEMS)
    ...
    ax.set_title('Accuracy by partial set size (N = 50) - No singletons')
    ...
    ax.legend((rects1[0], rects2[0], rects3[0], rects4[0]),
              ('Markov', 'Markov Heuristic', 'Modular', 'FLDC (L=20, K=20)'),
              loc='upper left')
    plt.savefig(os.path.join(
        constants.IMAGE_PATH, 'set_size_score_50_no_singles.eps'),
        bbox_inches='tight')
    plt.show()
